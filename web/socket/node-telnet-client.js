...
module.exports = class Telnet extends events.EventEmitter {
  ...
  connect(opts) {
    ...
    return promise = new Promise((_resolve, _reject) => {
      ...
      this.socket.on('data', data => {
        ...
        console.log(`Telnet.data '${stringData.substr(0, 20)}...'`);
        this.emit('data', data.toString());
        if (this.state === 'standby')
          return;

        this._parseData(data, (event, parsed) => {
          if (promise != null && event === 'ready') {
            resolve(parsed)
          }
        })
      })
  ...
  destroy() {
    this.socket != null && this.socket.destroy();
  }
  ...
  _parseData(chunk, callback) {
    ...
    if (this.state === 'getprompt') {
      ...
      console.log(`Telnet._responseready`);
      this.emit('responseready')
    }
  }

  _login(handle) {
    console.log(`Telnet._login ${handle}`);
