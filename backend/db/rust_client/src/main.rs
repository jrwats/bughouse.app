...
async fn main() -> Result<()> {
    ...
    // let res = session.query("DESCRIBE keyspaces", &[]).await?;
    // println!("DESCRIBE rows: {:?}, warnings: {:?}", res.rows, res.warnings);
    let cksp = session.query("CREATE KEYSPACE IF NOT EXISTS bughouse WITH REPLICATION = {'class' : 'SimpleStrategy', 'replication_factor' : 1}", &[]).await?;
    println!("rows: {:?}, warnings: {:?}", cksp.rows, cksp.warnings);
    // if let Some(rows) = session.query("SELECT * FROM system_schema.tables WHERE keyspace_name = bughouse", &[]).await?.rows {
